...
para_input<-read.csv(file = "C:/Users/sc7685/OneDrive - Princeton University/projects/5_LAAB/LAAB_hop/UploadGit/templates/LAAB_PrEP_template_solidtum.csv")
...
write.csv(x = data_output,file = "LAAB_PrEP_output_solidtumnew.csv")
...
p1<-ggplot(data=data_ggplot1,aes(x=lyg,y=cost))+
  ...
  scale_y_continuous(breaks=c(-30000000,-15000000,0,15000000,30000000),labels = comma,limit = c(-30000000, 30000000))+
  geom_line(aes(x=lyg,y=threshold_ICER*lyg),col="red",lwd=1)+
  geom_line(aes(x=lyg,y=100000*lyg),col="blue",lwd=1)+
...
p2<-ggplot(data=data_ggplot2,aes(x=buget,y=prob))+
  geom_line(size=1)+
  geom_vline(xintercept = threshold_ICER, color = "red", size=1,linetype="dashed")+
