#########################################################################
### A model for LAAB-PrEP cost-effectiveness analysis ###
### Script for plotting Fig 1 where shielding cost is not considered  ###
#########################################################################
rm(list = ls())        
set.seed(100)          
...
para_input<-read.csv(file = ".../templates/LAAB_PrEP_template.csv")
...
cs_v <- 0#seq(as.numeric(para_input$parameter1[para_input$symbol=="cs"]),as.numeric(para_input$parameter2[para_input$symbol=="cs"]),by=as.numeric(para_input$parameter3[para_input$symbol=="cs"]))                     
...
write.csv(x = data_output,file = "LAAB_PrEP_outputsnew.csv")

###ggplot for COST-EFFECTIVENESS plane and WTP curve ##
## Updated dots with lighter colors ##
...
p1<-ggplot(data=data_ggplot1,aes(x=lyg,y=cost))+
  ...
  scale_x_continuous(expand = expansion(mult = c(0, 0)),breaks=c(0,200,400,600,800,1000),labels = comma,limit = c(0, 1000))+
  scale_y_continuous(expand = expansion(mult = c(0, 0)),breaks=c(0,10000000,20000000,30000000),labels = comma,limit = c(0, 30000000))+
  geom_line(aes(x=lyg,y=threshold_ICER*lyg),col="red",lwd=1.5)+
  geom_line(aes(x=lyg,y=100000*lyg),col="blue",lwd=1.5)+
  geom_line(aes(x=lyg,y=20000*lyg),col="purple",lwd=1.5)+
  xlab(~ paste("Incremental QALYs, ",Delta,"Q")) + ylab(~ paste("Incremental costs (GBP), ",Delta,"C"))
...
p2<-ggplot(data=data_ggplot2,aes(x=buget,y=prob))+
  geom_line(size=1.5)+
  geom_vline(xintercept = 20000, color = "purple", size=1.5,linetype="dashed")+
  geom_vline(xintercept = 30000, color = "red", size=1.5,linetype="dashed")+
  geom_vline(xintercept = 100000, color = "blue", size=1.5,linetype="dashed")+
