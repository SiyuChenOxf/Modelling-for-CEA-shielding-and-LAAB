...
setwd("/Users/schen/Desktop/LAAB_hop/th")                                       
...
library(gridExtra)
library(ggpubr)
library(varhandle)  #for Lisa's BDI desktop 
library(dplyr)
library(gtools)
...
para_input<-read.csv(file = "AZD7442MathematicalModellingParameters_shielding_UKprophylaxis356_LJW_29MAY_s1.csv")

para_input<-unfactor(para_input)                                                                               #for Lisa's BID desktop; works on Siyu's desktop & Macbook
...
write.csv(x = data_output,file = "CEA_OUTPUTS_th10_s1_0.8.csv")

# data<-read.csv("CEA_OUTPUTS_05JUL_hobs.csv")

###ggplot for COST-EFFECTIVENESS plane and WTP curve##
indlarge<-which(rho>0.5)
indsmall<-which(rho<0.5 | rho == 0.5)
threshold_ICER<-30000 
data_ggplot1<-data.frame(lyg=LYG[indlarge],cost=COST[indlarge])
data_ggplot3<-data.frame(lyg=LYG[indsmall],cost=COST[indsmall])

wtp<-k<-seq(from=0,to=80000,by=1000)
for (i in 1:length(k)) {
  wtp[i]<-sum(LYG[indlarge]*k[i] > COST[indlarge])/length(LYG[indlarge])
}
data_ggplot2<-data.frame(buget=k,prob=wtp)
wtp<-k<-seq(from=0,to=80000,by=1000)
for (i in 1:length(k)) {
  wtp[i]<-sum(LYG[indsmall]*k[i] > COST[indsmall])/length(LYG[indsmall])
}
data_ggplot4<-data.frame(buget=k,prob=wtp)

p1<-ggplot()+
  geom_point(data=data_ggplot1,aes(x=lyg,y=cost),col="blue")+
  geom_hline(yintercept = 0) +
  geom_vline(xintercept = 0)+
  scale_x_continuous(label=comma,limit = c(min(data_ggplot1$lyg), max(data_ggplot1$lyg)))+
  # scale_y_continuous(label=comma,limit = c(min(data_ggplot1$cost), max(data_ggplot1$cost)))+
  scale_y_continuous(breaks=c(-15000000,-10000000,-5000000,0,5000000,10000000,15000000),labels = comma,limit = c(-15000000, 15000000))+
  geom_line(data=data_ggplot1,aes(x=lyg,y=threshold_ICER*lyg),col="red",lwd=1)+
  xlab("Life years gained")+ylab("Costs (GBP)")+
  geom_point(data=data_ggplot3,aes(x=lyg,y=cost),col="black")
...
p2<-ggplot()+
  geom_line(data=data_ggplot2,aes(x=buget,y=prob),size=1,col="blue")+
  ...
  geom_line(data=data_ggplot4,aes(x=buget,y=prob),col="black")
...
p1<-ggplot(data=data_ggplot1,aes(x=lyg,y=cost))+
  ...
  xlab("Quality adjusted life years gained") + ylab("Incremental costs (GBP)")
